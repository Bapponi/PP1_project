// generated with ast extension for cup
// version 0.8
// 16/0/2024 20:44:56


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(FormPars FormPars) { }
    public void visit(StatementListB StatementListB) { }
    public void visit(NamespaceDots NamespaceDots) { }
    public void visit(Factor Factor) { }
    public void visit(Statement Statement) { }
    public void visit(Brackets Brackets) { }
    public void visit(FactorDesignatorBodyListB FactorDesignatorBodyListB) { }
    public void visit(ExprListB2 ExprListB2) { }
    public void visit(VoidOrType VoidOrType) { }
    public void visit(FormParsListB FormParsListB) { }
    public void visit(ConstDeclBodyList ConstDeclBodyList) { }
    public void visit(Relop Relop) { }
    public void visit(FormBodyList FormBodyList) { }
    public void visit(VarDeclBodyList VarDeclBodyList) { }
    public void visit(BracketsList BracketsList) { }
    public void visit(Expr Expr) { }
    public void visit(NumConstListB NumConstListB) { }
    public void visit(NewTypeBody NewTypeBody) { }
    public void visit(MaybeDesignator MaybeDesignator) { }
    public void visit(FormBody FormBody) { }
    public void visit(TypeList TypeList) { }
    public void visit(AllConst AllConst) { }
    public void visit(ActParsBodyList ActParsBodyList) { }
    public void visit(TermBodyList TermBodyList) { }
    public void visit(Condition Condition) { }
    public void visit(DesignatorListB DesignatorListB) { }
    public void visit(Mulop Mulop) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(NamespaceList NamespaceList) { }
    public void visit(AllDeclList AllDeclList) { }
    public void visit(DesignatorStarList DesignatorStarList) { }
    public void visit(Addop Addop) { }
    public void visit(StatementList StatementList) { }
    public void visit(ExprMP ExprMP) { }
    public void visit(ClassDeclBody ClassDeclBody) { }
    public void visit(DesignatorBodyList DesignatorBodyList) { }
    public void visit(ExprBodyList ExprBodyList) { }
    public void visit(ExprListB ExprListB) { }
    public void visit(CondTerm CondTerm) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(AllDecl AllDecl) { }
    public void visit(ExtendsTypeListB ExtendsTypeListB) { }
    public void visit(ActPars ActPars) { }
    public void visit(ActParsBody ActParsBody) { }
    public void visit(MaybeCondFact MaybeCondFact) { }
    public void visit(DesignatorStatementListB DesignatorStatementListB) { }
    public void visit(VarDeclList VarDeclList) { }
    public void visit(CondFact CondFact) { }
    public void visit(ActParsListB ActParsListB) { }
    public void visit(MinusListB MinusListB) { }
    public void visit(SingleCondFact SingleCondFact) { visit(); }
    public void visit(DoubleCondFact DoubleCondFact) { visit(); }
    public void visit(CondTermSingle CondTermSingle) { visit(); }
    public void visit(CondTermAll CondTermAll) { visit(); }
    public void visit(ConditionSingle ConditionSingle) { visit(); }
    public void visit(ConditionAll ConditionAll) { visit(); }
    public void visit(NoNumConstListB NoNumConstListB) { visit(); }
    public void visit(AllNumConstListB AllNumConstListB) { visit(); }
    public void visit(RelopLesseqv RelopLesseqv) { visit(); }
    public void visit(RelopLess RelopLess) { visit(); }
    public void visit(RelopMoreeqv RelopMoreeqv) { visit(); }
    public void visit(RelopMore RelopMore) { visit(); }
    public void visit(RelopNoeqv RelopNoeqv) { visit(); }
    public void visit(RelopEqv RelopEqv) { visit(); }
    public void visit(Label Label) { visit(); }
    public void visit(AddopMinus AddopMinus) { visit(); }
    public void visit(AddopPlus AddopPlus) { visit(); }
    public void visit(SingleActPars SingleActPars) { visit(); }
    public void visit(AllActPars AllActPars) { visit(); }
    public void visit(NoActParsListB NoActParsListB) { visit(); }
    public void visit(SingleActParsListB SingleActParsListB) { visit(); }
    public void visit(NewTypeBodyTwo NewTypeBodyTwo) { visit(); }
    public void visit(NewTypeBodyOne NewTypeBodyOne) { visit(); }
    public void visit(FactorDesignatorBody FactorDesignatorBody) { visit(); }
    public void visit(NoFactorDesignatorBodyListB NoFactorDesignatorBodyListB) { visit(); }
    public void visit(SingleFactorDesignatorBodyListB SingleFactorDesignatorBodyListB) { visit(); }
    public void visit(Rparen Rparen) { visit(); }
    public void visit(Lparen Lparen) { visit(); }
    public void visit(FactorHashDes FactorHashDes) { visit(); }
    public void visit(FactorHash FactorHash) { visit(); }
    public void visit(FactorExpr FactorExpr) { visit(); }
    public void visit(FactorNew FactorNew) { visit(); }
    public void visit(FactorBool FactorBool) { visit(); }
    public void visit(FactorChar FactorChar) { visit(); }
    public void visit(FactorNum FactorNum) { visit(); }
    public void visit(FactorDesignatorAll FactorDesignatorAll) { visit(); }
    public void visit(MulopMod MulopMod) { visit(); }
    public void visit(MulopDiv MulopDiv) { visit(); }
    public void visit(MulopMul MulopMul) { visit(); }
    public void visit(TermBody TermBody) { visit(); }
    public void visit(NoTermBodyList NoTermBodyList) { visit(); }
    public void visit(AllTermBodyList AllTermBodyList) { visit(); }
    public void visit(Term Term) { visit(); }
    public void visit(ExprBody ExprBody) { visit(); }
    public void visit(NoExprBodyList NoExprBodyList) { visit(); }
    public void visit(AllExprBodyList AllExprBodyList) { visit(); }
    public void visit(ExprPlus ExprPlus) { visit(); }
    public void visit(ExprMinus ExprMinus) { visit(); }
    public void visit(NoExprListB NoExprListB) { visit(); }
    public void visit(SingleExprListB SingleExprListB) { visit(); }
    public void visit(Assignop Assignop) { visit(); }
    public void visit(DesignatorBody DesignatorBody) { visit(); }
    public void visit(NoDesignatorBodyList NoDesignatorBodyList) { visit(); }
    public void visit(AllDesignatorBodyList AllDesignatorBodyList) { visit(); }
    public void visit(DesignatorName DesignatorName) { visit(); }
    public void visit(Designator Designator) { visit(); }
    public void visit(SingleExprListB2 SingleExprListB2) { visit(); }
    public void visit(AllExprListB2 AllExprListB2) { visit(); }
    public void visit(SingleDesignatorListB SingleDesignatorListB) { visit(); }
    public void visit(AllDesignatorListB AllDesignatorListB) { visit(); }
    public void visit(DesignatorStatementMultiple DesignatorStatementMultiple) { visit(); }
    public void visit(NoDesignator NoDesignator) { visit(); }
    public void visit(YesDesignator YesDesignator) { visit(); }
    public void visit(NoDesignatorStarList NoDesignatorStarList) { visit(); }
    public void visit(AllDesignatorStarList AllDesignatorStarList) { visit(); }
    public void visit(DesignatorStarBody DesignatorStarBody) { visit(); }
    public void visit(DesignatorStatementStar DesignatorStatementStar) { visit(); }
    public void visit(DesignatorStatementDec DesignatorStatementDec) { visit(); }
    public void visit(DesignatorStatementInc DesignatorStatementInc) { visit(); }
    public void visit(DesignatorStatementAct DesignatorStatementAct) { visit(); }
    public void visit(DesignatorStatementAssign DesignatorStatementAssign) { visit(); }
    public void visit(NoDesignatorStmListB NoDesignatorStmListB) { visit(); }
    public void visit(SingleDesignatorStmListB SingleDesignatorStmListB) { visit(); }
    public void visit(AllDesignatorStmListB AllDesignatorStmListB) { visit(); }
    public void visit(NoCondFact NoCondFact) { visit(); }
    public void visit(YesCondFact YesCondFact) { visit(); }
    public void visit(ForBody ForBody) { visit(); }
    public void visit(For For) { visit(); }
    public void visit(Foreach Foreach) { visit(); }
    public void visit(While While) { visit(); }
    public void visit(CondStart CondStart) { visit(); }
    public void visit(EndFor EndFor) { visit(); }
    public void visit(EndElse EndElse) { visit(); }
    public void visit(Else Else) { visit(); }
    public void visit(EndIf EndIf) { visit(); }
    public void visit(ErrorStmt ErrorStmt) { visit(); }
    public void visit(StatementArray2 StatementArray2) { visit(); }
    public void visit(StatementArray StatementArray) { visit(); }
    public void visit(StatementFor StatementFor) { visit(); }
    public void visit(StatementFindAndReplace StatementFindAndReplace) { visit(); }
    public void visit(StatementFindAny StatementFindAny) { visit(); }
    public void visit(StatementStatementList StatementStatementList) { visit(); }
    public void visit(StatementForeach StatementForeach) { visit(); }
    public void visit(StatementPrint StatementPrint) { visit(); }
    public void visit(StatementRead StatementRead) { visit(); }
    public void visit(StatementReturn StatementReturn) { visit(); }
    public void visit(StatementContinue StatementContinue) { visit(); }
    public void visit(StatementBreak StatementBreak) { visit(); }
    public void visit(StatementWhile StatementWhile) { visit(); }
    public void visit(StatementElse StatementElse) { visit(); }
    public void visit(StatementIf StatementIf) { visit(); }
    public void visit(StatementDesignMultiple StatementDesignMultiple) { visit(); }
    public void visit(StatementDesign StatementDesign) { visit(); }
    public void visit(NoStatementList NoStatementList) { visit(); }
    public void visit(AllStatementList AllStatementList) { visit(); }
    public void visit(FormParsBodyType FormParsBodyType) { visit(); }
    public void visit(FormParsBody FormParsBody) { visit(); }
    public void visit(SingleFormPars SingleFormPars) { visit(); }
    public void visit(AllFormPars AllFormPars) { visit(); }
    public void visit(NoFormParsListB NoFormParsListB) { visit(); }
    public void visit(SingleFormParsListB SingleFormParsListB) { visit(); }
    public void visit(TypePart TypePart) { visit(); }
    public void visit(VoidPart VoidPart) { visit(); }
    public void visit(MethodTypeName MethodTypeName) { visit(); }
    public void visit(MethodDecl MethodDecl) { visit(); }
    public void visit(NoMethodDeclListList NoMethodDeclListList) { visit(); }
    public void visit(AllMethodDeclList AllMethodDeclList) { visit(); }
    public void visit(NoClassDeclBody NoClassDeclBody) { visit(); }
    public void visit(SingleClassDeclBody SingleClassDeclBody) { visit(); }
    public void visit(NoVarDeclList NoVarDeclList) { visit(); }
    public void visit(AllVarDeclList AllVarDeclList) { visit(); }
    public void visit(NoExtendsType NoExtendsType) { visit(); }
    public void visit(SingleExtendsType SingleExtendsType) { visit(); }
    public void visit(ClassName ClassName) { visit(); }
    public void visit(ClassDecl ClassDecl) { visit(); }
    public void visit(VarDeclBody VarDeclBody) { visit(); }
    public void visit(NoVarDeclBodyList NoVarDeclBodyList) { visit(); }
    public void visit(AllVarDeclBodyList AllVarDeclBodyList) { visit(); }
    public void visit(NoBrackets NoBrackets) { visit(); }
    public void visit(SingleBrackets SingleBrackets) { visit(); }
    public void visit(VarDeclTypeName VarDeclTypeName) { visit(); }
    public void visit(VarDecl VarDecl) { visit(); }
    public void visit(ConstDeclBody ConstDeclBody) { visit(); }
    public void visit(NoConstDeclBodyList NoConstDeclBodyList) { visit(); }
    public void visit(AllConstDeclBodyList AllConstDeclBodyList) { visit(); }
    public void visit(Num Num) { visit(); }
    public void visit(Bool Bool) { visit(); }
    public void visit(Char Char) { visit(); }
    public void visit(AllConstNum AllConstNum) { visit(); }
    public void visit(AllConstBool AllConstBool) { visit(); }
    public void visit(AllConstChar AllConstChar) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(ConstDeclTypeName ConstDeclTypeName) { visit(); }
    public void visit(ConstDecl ConstDecl) { visit(); }
    public void visit(ClassDeclarations ClassDeclarations) { visit(); }
    public void visit(VarDeclarations VarDeclarations) { visit(); }
    public void visit(ConstDeclarations ConstDeclarations) { visit(); }
    public void visit(NoAllDecl NoAllDecl) { visit(); }
    public void visit(AllDeclarationsList AllDeclarationsList) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(NamespaceName NamespaceName) { visit(); }
    public void visit(Namespace Namespace) { visit(); }
    public void visit(NoNamespaceList NoNamespaceList) { visit(); }
    public void visit(AllNamespaceList AllNamespaceList) { visit(); }
    public void visit(ProgramBody ProgramBody) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
